...
#define	FCLK_SLOW()	setupSlow()		/* Set slow clock, 100k-400k (Nothing to do) */
#define	FCLK_FAST()	setupFast()		/* Set fast clock, depends on the CSD (Nothing to do) */
...
void power_on (void)
{
    ...
    SPI1CON1bits.SMP = 1; // Input data is sampled at the middle of data
    // output time
...
void setupSlow()
{
    /* Set slow clock (100k-400k) *///i.e 312.5khz
